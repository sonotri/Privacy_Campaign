(() => {
  ...
  const cards = [
    ...
    },
    {
      ...
      stars: 1,
      file: "hobby_card.png",
      detail: "내 취향과 관심을 보여주는 정보",
    },
    {
      id: "birthday",
      name: "생년월일",
      stars: 2,
      file: "birthday_card.png",
      detail: "나이와 생일을 알려주는 정보",
    },
    {
      id: "school",
      name: "학교/소속",
      stars: 2,
      file: "school_affiliation_card.png",
      detail: "내가 속한 학교나 단체에 관한 정보",
    },
    {
      id: "email",
      name: "이메일 주소",
      stars: 3,
      file: "email_card.png",
      detail: "연락과 계정 연결에 쓰이는 주소",
    },
    {
      id: "residence",
      name: "거주지 정보",
      stars: 3,
      file: "residence_card.png",
      detail: "내가 생활하는 지역을 보여주는 정보",
    },
  ];
  const symbols = [
    { name: "백조", file: "swan.png" },
    { name: "열쇠", file: "key.png" },
    { name: "별", file: "star.png" },
  ];
  const {
    facts: exampleFacts,
    casesFor: buildCases,
    chapters,
  } = window.BlackSwanStory;
  ...
  const personal = window.BlackSwanPersonal.create({
    examples: exampleFacts,
    onChange: refreshEntry,
  });
  ...
  const usedCards = () =>
    state.used.map((id) => cards.find((card) => card.id === id));
  const availableCards = () =>
    cards.filter((card) => !state.used.includes(card.id));
  const bonus = () =>
    cards
      .filter((card) => state.selected.includes(card.id))
      .reduce((sum, card) => sum + card.stars, 0);
  ...
  function beep(frequency = 500, duration = 0.08, type = "sine") {
  ...
  function go(screen) {
    ...
    state.screen = screen;
    render();
    if (!app.querySelector("dialog[open]")) app.focus({ preventScroll: true });
  ...
  function cardMarkup(card, mode = "select") {
    const used = state.used.includes(card.id),
  ...
  function playScreen() {
    ...
    const initialSpin = state.rounds === 0 && !receipt;
    return `<section class="play-screen immersive-play"><div class="game-hud"><div><span class="eyebrow">YOUR CHANCE</span><div class="spin-count"><strong>${String(state.spins).padStart(2, "0")}</strong><span>SPINS LEFT</span></div></div><div class="round-count">ROUND <b>${String(state.rounds + (!state.spinning && !missed ? 1 : 0)).padStart(2, "0")}</b><span>남은 카드 ${availableCards().length} / 6</span></div></div>
      ...
      <div class="round-feedback" aria-live="polite"><div class="round-message"><span>${receipt ? "TRADE CONFIRMED" : initialSpin ? "FREE SPIN" : offer ? "BLACK SWAN" : exhausted ? "LAST SPIN" : missed ? "MISS" : " "}</span><p>${receipt ? `${receipt.name} → <strong>SPIN +${receipt.stars}</strong>` : initialSpin ? "무료 1번 기회" : offer ? "조금 더 가치 있는 정보라면,<br><strong>BONUS SPIN!</strong>" : exhausted ? "모든 기회를 사용했습니다.<br>이제, 당신의 선택을 확인하세요." : missed && state.spins ? `아직 <strong>${state.spins}번</strong>의 기회가 남아 있습니다.` : missed ? "남은 SPIN을 모두 사용했습니다.<br>카드 한 장으로 기회를 이어갈까요?" : ""}</p></div>${missed && state.spins ? '<button class="quiet-primary" data-action="spin">한 번 더 SPIN</button>' : exhausted ? '<button class="quiet-primary" data-action="finish">나의 선택 확인</button>' : ""}</div>
      <div class="play-bottom">${offer ? "<span>★★ 총 2회 · ★★★ 총 3회</span>" : ""}${!exhausted ? `<button class="text-button" data-action="${canTrade ? "cancel" : "finish"}" ${state.spinning ? "disabled" : ""}>여기서 마치기</button>` : ""}</div></section>${canTrade ? exchangeDialog() : ""}`;
  }
  ...
  function jackpotScreen() {
    return `<section class="jackpot-screen"><div class="jackpot-sparks" aria-hidden="true">${Array.from({ length: 16 }, (_, i) => `<i style="--i:${i}">✦</i>`).join("")}</div><button class="jackpot-touch" data-action="finish"><span class="eyebrow">THREE OF A KIND</span><h1>JACKPOT!</h1>${resultSymbols()}<span class="reward-badge">${icon} 당첨 ${icon}</span><p>화면을 운영진에게 보여주세요.</p><span class="small">당첨으로 룰렛이 종료됩니다. 남은 SPIN ${state.spins}회</span><span class="touch-prompt">화면을 터치해주세요</span></button></section>${footer(1)}`;
  }
  function revealScreen() {
    const count = state.used.length;
    return `<section class="reveal-scene"><div class="reveal-kicker"><span>CONSENT TEST</span></div><div class="reveal-main"><h2>Game Over.<br><em>이제 선택의 결과를 확인할 차례입니다.</em></h2><p class="reveal-line">${count ? `총 <strong>${count}장의 정보</strong>를 건넸습니다.<br>내가 건넨 정보가 어떤 의미를 가질 수 있는지 확인해볼까요?.` : "이번 게임에서는 개인정보를 교환하지 않았습니다."}</p></div><button class="story-next" data-action="used">${count ? "내가 건넨 정보 보기" : "나의 선택 돌아보기"}<span aria-hidden="true">→</span></button><span class="scene-footnote">${state.won ? " " : count ? " " : "이번 게임에서 사용한 개인정보 카드 0장"}</span></section>`;
  }
  function usedScreen() {
    const used = usedCards(),
      earned = used.reduce((sum, card) => sum + card.stars, 0);
    const kept = availableCards();
    return `<section class="evidence-screen"><header class="evidence-header"><span class="scene-label">01 / YOUR EXCHANGE</span><h1>${used.length ? `<em>${earned}번의 기회와 ${used.length}장의 정보.</em><br> 이번 게임에서 주고받은 것들입니다.` : "여섯 장 모두,<br>당신에게 남았습니다."}</h1><p>${used.length ? " " : "정보를 제공하지 않고 게임을 마쳤습니다."}</p></header>
      ...
      <div class="evidence-question"><h3>${used.length >= 2 ? " " : used.length ? "이 한 장은,<br>어떤 단서로 남을까요?" : "다음에 정보를 요청받는다면,<br>무엇을 먼저 확인할까요?"}</h3><button class="story-next" data-action="combine">${used.length >= 2 ? "정보 연결하기" : "이 선택의 의미 보기"}<span aria-hidden="true">→</span></button></div></section>`;
  }
  ...
  function graphMarkup(used) {
    const positions =
      used.length === 1
        ? [[50, 14]]
        : used.map((_, i) => {
            const a = -Math.PI / 2 + (i * Math.PI * 2) / used.length;
            return [50 + Math.cos(a) * 35, 50 + Math.sin(a) * 35];
          });
    const pairs = new Set();
    casesFor(state.used).forEach((rule) =>
      rule.ids.forEach((id, i) =>
        rule.ids
          ...
          .forEach((other) =>
            pairs.add(
              [
                used.findIndex((c) => c.id === id),
                used.findIndex((c) => c.id === other),
              ]
                .sort((a, b) => a - b)
                .join(":"),
            ),
    ...
    return `<div class="data-graph"><svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">${positions.map(([x, y]) => `<line x1="${x}" y1="${y}" x2="50" y2="50"/>`).join("")}${[
      ...pairs,
    ]
      .map((pair) => {
        const [a, b] = pair.split(":").map(Number);
        return `<line class="data-link" x1="${positions[a][0]}" y1="${positions[a][1]}" x2="${positions[b][0]}" y2="${positions[b][1]}"/>`;
      })
      .join(
        "",
      )}</svg><div class="graph-center"><span>PROFILE</span><small>${used.length} DATA</small></div>${used.map((card, i) => `<div class="graph-node" style="--x:${positions[i][0]}%;--y:${positions[i][1]}%;--delay:${i * 0.14}s"><strong>${card.name}</strong></div>`).join("")}${used.length === 0 ? '<p class="graph-empty">연결할 개인정보가 없습니다.</p>' : ""}</div>`;
  }
  function combinedScreen(loading) {
    const used = usedCards();
    if (loading)
      return `<section class="linking-scene"><span class="scene-label">02 / CONNECTING THE DOTS</span><h1>${used.length > 1 ? "COMBINING DATA" : "당신의 선택을 살펴봅니다"}</h1>${graphMarkup(used)}<p>${used.length > 1 ? "정보는 하나로 존재하지 않습니다. 서로 연결되며 더 많은 의미를 만듭니다." : used.length ? "정보의 의미는, 쓰이는 맥락에 따라 달라집니다." : "이번에는 연결할 개인정보가 없습니다."}</p><button class="text-button" data-action="show-conclusion">결과 바로 보기</button></section>`;
    const cases = casesFor(state.used),
      item = cases[state.caseIndex];
  ...
  function render() {
    const screens = {
      ...
      reveal: revealScreen,
      used: usedScreen,
      combining: () => combinedScreen(true),
      combined: () => combinedScreen(false),
    ...
    if (exchange) {
      exchange.showModal();
      exchange.addEventListener("cancel", (event) => {
    ...
    document.body.classList.toggle(
      "in-story",
      ["reveal", "used", "combining", "combined", "credits", "ended"].includes(
        state.screen,
      ),
    );
    if (state.screen === "combining")
      later(
        () => {
          if (state.screen === "combining") go("combined");
        },
    ...
    if (state.screen === "jackpot")
      later(
        () =>
          document
            .querySelector(".jackpot-touch")
  ...
  function drawOutcome(distinctSymbols) {
    ...
    const remaining = symbols.map((_, index) => index),
      outcome = [];
    while (remaining.length)
      outcome.push(remaining.splice(randomIndex(remaining.length), 1)[0]);
    return outcome;
  }
  async function rollReel(index, result) {
    ...
    if (!reel) return;
    const finalImage = () =>
      `<img src="${asset(symbols[result].file)}" alt="${index + 1}번 심볼: ${symbols[result].name}" width="1254" height="1254">`;
    ...
    if (reducedMotion || !reel.animate) {
      ...
      const sequence = Array.from({ length: turns + 1 }, (_, step) =>
        step === turns ? result : (state.reels[index] + step) % symbols.length,
      );
      ...
      strip.innerHTML = sequence
        .map(
          (value) =>
            `<img src="${asset(symbols[value].file)}" alt="" width="1254" height="1254" draggable="false">`,
        )
        .join("");
      ...
      const animation = strip.animate(
        [
          ...
          {
            transform: `translateY(-${distance * 0.1}%)`,
            offset: 0.18,
            easing: "linear",
          },
          {
            transform: `translateY(-${distance * 0.78}%)`,
            offset: 0.7,
            easing: "cubic-bezier(.1,.55,.3,1)",
          },
          {
            transform: `translateY(-${distance + 4}%)`,
            offset: 0.965,
            easing: "ease-out",
          },
      ...
      try {
        ...
        /* Still settle on the drawn result if interrupted. */
      }
      animation.cancel();
    }
    ...
    reel.setAttribute(
      "aria-label",
      `${index + 1}번 심볼: ${symbols[result].name}`,
    );
    beep(420 + index * 210, 0.18, "triangle");
    const status = document.querySelector(".spin-status");
    if (status)
      status.textContent =
        index === 2
          ? "세 개의 심볼을 확인하세요!"
          : `${index + 1}번째 심볼 확정 · ${symbols[result].name}`;
  }
  async function spin() {
    if (
      !["play", "miss"].includes(state.screen) ||
      state.spinning ||
      state.spins < 1
    )
      return;
    ...
    const outcome = drawOutcome(state.rounds <= 2);
    render();
    say("룰렛을 돌립니다.");
    ...
    const cycle = reducedMotion
      ? null
      : setInterval(() => {
          beep(160 + (tick++ % 5) * 42, 0.035, "triangle");
        }, 105);
    try {
      await Promise.all(
        outcome.map((result, index) => rollReel(index, result)),
      );
    } finally {
      clearInterval(cycle);
    }
    ...
    state.won = outcome.every((value) => value === outcome[0]);
    say(
      `${outcome.map((i) => symbols[i].name).join(", ")}. ${state.won ? "잭팟! 상품에 당첨되었습니다." : "미당첨입니다."}`,
    );
    go(state.won ? "jackpot" : "miss");
    if (state.won)
      [523, 659, 784, 1046].forEach((hz, i) =>
        later(() => beep(hz, 0.25), i * 140),
      );
  }
  function refreshEntry() {
    ...
    button.textContent = card
      ? `이 정보로 SPIN ${card.stars}회 교환`
      : "입력한 정보로 교환";
    if (card)
      document.querySelector(
        `[data-card="${card.id}"] .personal-card-art`,
      ).outerHTML = personal.art(card, asset(card.file));
  }
  function toggleCard(id, focus = true) {
    if (
      state.screen !== "trade" ||
      state.spins !== 0 ||
      state.used.includes(id) ||
    ...
    const card = cards.find((card) => state.selected.includes(card.id));
    if (card && focus)
      document.getElementById("card-value")?.focus({ preventScroll: false });
    beep(600, 0.04);
    say(
      card
        ? `${card.name} 선택. 내용을 직접 작성한 뒤 교환하세요.`
        : "선택을 해제했습니다.",
    );
  }
  function confirmTrade() {
    ...
    const card = cards.find(
      (card) => card.id === state.selected[0] && !state.used.includes(card.id),
    );
    if (!card || !personal.confirm(card.id)) return;
    state.used.push(card.id);
    state.lastTrade = card.id;
    state.selected = [];
    state.spins += card.stars;
    state.receipt = card.id;
    go("play");
    beep(740, 0.2, "triangle");
    say(`교환 완료. ${card.name}, SPIN ${card.stars}회.`);
  }
  async function action(name) {
    if (state.spinning) return;
    switch (name) {
      case "start":
        if (state.screen === "home") go("how");
        break;
      case "play":
        if (["how", "miss", "cancel"].includes(state.screen)) {
          state.selected = [];
          go(state.spins ? "play" : "trade");
        }
        break;
      case "spin":
        await spin();
      ...
      case "trade":
        if (
          ["miss", "cancel"].includes(state.screen) &&
          state.spins === 0 &&
      ...
      case "confirm-trade":
        ...
        break;
      case "cancel":
        if (["trade", "miss"].includes(state.screen)) {
          personal.clearDraft();
          state.selected = [];
          go("cancel");
        }
        break;
      case "finish":
        if (["play", "miss", "cancel", "jackpot"].includes(state.screen)) {
          state.selected = [];
          go("reveal");
        }
        break;
      case "used":
        if (state.screen === "reveal") go("used");
        break;
      case "combine":
        if (state.screen === "used") {
          state.caseIndex = 0;
          go("combining");
        }
        break;
      ...
      case "case-next":
        if (
          state.screen === "combined" &&
          state.caseIndex < casesFor(state.used).length - 1
        ) {
          state.caseIndex++;
          go("combined");
        }
        break;
      case "case-prev":
        if (state.screen === "combined" && state.caseIndex > 0) {
          state.caseIndex--;
          go("combined");
        }
        break;
      case "credits":
        if (state.screen === "combined") {
          state.chapter = 0;
          go("credits");
        }
        break;
      case "end":
        if (state.screen === "credits") go("ended");
        break;
      case "read-again":
  ...
  app.addEventListener("click", (event) => {
    if (state.screen === "home" && event.target.closest(".home-art")) {
      void action("start");
      return;
    }
    const card = event.target.closest("[data-card]");
    if (card && !card.disabled) return toggleCard(card.dataset.card);
  ...
  document.getElementById("sound-toggle").addEventListener("click", (event) => {
    ...
    button.setAttribute(
      "aria-label",
      soundOn ? "음악과 효과음 끄기" : "음악과 효과음 켜기",
    );
    button.querySelector("span").textContent = soundOn ? "ON" : "OFF";
    beep(660, 0.15);
  });
  ...
  fullscreen.addEventListener("click", async () => {
    try {
      ...
      say("현재 브라우저에서는 전체 화면을 사용할 수 없습니다.");
    }
  });
  ...
  document.addEventListener("keydown", (event) => {
    ...
    if (
      ...
      event.preventDefault();
      void action(event.key === "ArrowLeft" ? "case-prev" : "case-next");
    }
  });
  ...
  if (document.modelContext?.registerTool) {
    ...
    register({
      name: "read_roulette_state",
      title: "현재 게임 상태 확인",
      description: "현재 화면, 남은 SPIN과 선택·교환한 카드 종류를 읽습니다.",
      inputSchema: {
        type: "object",
        properties: {},
    ...
    register({
      name: "advance_roulette",
      title: "게임 화면에서 행동하기",
      description:
        "현재 표시된 행동을 실행합니다. confirm-trade는 카드를 소비하고 SPIN을 추가합니다. spin은 SPIN 1회를 소비하고 결과까지 기다립니다.",
      inputSchema: {
        type: "object",
        properties: {
          action: {
      ...
      async execute(input) {
        if (
          ...
          !Array.from(app.querySelectorAll("[data-action]")).some(
            (button) =>
              button.dataset.action === input.action && !button.disabled,
          )
        )
          throw new Error("현재 화면에서 사용할 수 없는 행동입니다.");
        await action(input.action);
        return snapshot();
      },
    });
    register({
      name: "select_privacy_cards",
      title: "교환할 카드 선택",
      description:
        "카드 한 장만 선택합니다. 실제 교환은 confirm-trade 행동으로 완료합니다.",
      inputSchema: {
        type: "object",
        properties: {
          cardIds: {
            ...
            maxItems: 1,
            uniqueItems: true,
            items: { type: "string", enum: cards.map((card) => card.id) },
          },
        },
        required: ["cardIds"],
        additionalProperties: false,
      ...
      execute(input) {
        if (
          state.screen !== "trade" ||
          !input ||
          !Array.isArray(input.cardIds) ||
          input.cardIds.length > 1 ||
          state.spins !== 0 ||
          Object.keys(input).some((key) => key !== "cardIds") ||
          new Set(input.cardIds).size !== input.cardIds.length ||
          input.cardIds.some(
            (id) => !availableCards().some((card) => card.id === id),
          )
        )
          throw new Error("현재 교환 가능한 카드만 선택할 수 있습니다.");
        personal.select(input.cardIds[0]);
        state.selected = [...input.cardIds];
        render();
        refreshEntry();
        return snapshot();
      },
