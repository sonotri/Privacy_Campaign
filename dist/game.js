(() => {
  ...
  function usedScreen() {
    ...
    return `<section class="evidence-screen"><header class="evidence-header"><span class="scene-label">01 / YOUR EXCHANGE</span><h1>${used.length ? `<em>${earned}번의 기회와 ${used.length}장의 정보.</em><br> 이번 게임에서 주고받은 것들입니다.` : "여섯 장 모두,<br>당신에게 남았습니다."}</h1><p>${used.length ? "보상을 기다리는 동안, 다음 정보들이 같은 사람의 기록으로 모였습니다." : "정보를 제공하지 않고 게임을 마쳤습니다."}</p></header>
      ...
      <div class="evidence-question"><h2>${used.length >= 2 ? "하지만, 따로 건넨 정보가<br>서로 연결된다면?" : used.length ? "이 한 장은,<br>어떤 단서로 남을까요?" : "다음에 정보를 요청받는다면,<br>무엇을 먼저 확인할까요?"}</h2><button class="story-next" data-action="combine">${used.length >= 2 ? "정보 연결하기" : "이 선택의 의미 보기"}<span aria-hidden="true">→</span></button></div></section>`;
  }
